...
class dx_iface : public DRW_Interface {
 public:
  ...
  bool fileImport(const boost::filesystem::path& path, dx_data* fData);
  ...
  // header
  void addHeader(const DRW_Header* data) { cData->headerC = *data; }
  ...
  virtual void endBlock() { currentBlock = cData->mBlock; }
  ...
  virtual void writeHeader(DRW_Header& data) {
    ...
    data = cData->headerC;
    // or copy one by one:
    //        for (std::map<std::string,DRW_Variant*>::iterator
    //        it=cData->headerC.vars.begin(); it != cData->headerC.vars.end();
    //        ++it)
