...
void usage() {
  ...
  std::cout << "   dwg2dxf <input> [-b] <-version> <output>" << std::endl
            << std::endl;
  std::cout << "   input      existing file to convert" << std::endl;
  std::cout << "   -b         optional, sets output as binary dxf" << std::endl;
  std::cout << "   -B         optional, batch mode reads a text file whit a "
               "list of full path input"
            << std::endl;
  std::cout << "               files and saves with the same name in the "
               "indicated folder as output"
            << std::endl;
  std::cout << "   -y -Y      optional, Warning! if output dxf exist overwrite "
               "without ask"
            << std::endl;
...
bool convertFile(std::string inName,
                 std::string outName,
                 DRW::Version ver,
                 bool binary,
                 bool overwrite) {
  ...
  if (!badState) {
    /*
    if (!overwrite){
        std::cout << "File " << outName << " already exist, overwrite Y/N ?" <<
    std::endl; int c = getchar(); if (c == 'Y' || c=='y')
            ;
        else {
            std::cout << "Cancelled.";
            return false;
...
int main(int argc, char* argv[]) {
  ...
  if (dirStat != 0 || S_ISDIR(statBuf.st_mode) == 0) {
    std::cout << "Batch mode: " << outName << " must be an existing directory"
              << std::endl;
    usage();
    return 4;
  }
  ...
  for (std::list<std::string>::const_iterator it = inList.begin();
       it != inList.end(); ++it) {
    std::string input = *it;
